        ...
        if (StreamKind_Last==Stream_Video && Retrieve(Stream_Video, StreamPos_Last, Video_Format)==__T("DV"))
        {
            ...
            for (size_t Audio_Pos=0; Audio_Pos<Audio_Count; Audio_Pos++)
            {
                ...
                if (Retrieve(Stream_Audio, Pos, Audio_MuxingMode).empty())
                    ...
                    Fill(Stream_Audio, Pos, Audio_MuxingMode, Retrieve(Stream_Video, Essence->second.StreamPos, Video_Format)+__T(" / ")+Retrieve(Stream_Audio, Pos, Audio_MuxingMode), true);
...
void File_Mxf::GenericPictureEssenceDescriptor_ActiveFormatDescriptor()
{
    ...
    int8u Data;
    BS_Begin();
    bool Is1dot3=Retrieve(Stream_General, 0, General_Format_Profile).To_float32()>=1.3?true:false;
    if (!Is1dot3)
    {
        int8u Probe;
        Peek_S1(2, Probe);
