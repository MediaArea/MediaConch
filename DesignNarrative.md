...
The PREFORMA challenge illuminates and responds to a significant and real obstacle that faces the preservation community today. This report details MediaArea’s design plan to create a toolset (tentatively entitled “MediaConch”) consisting of an implementation checker, policy checker, reporter, and fixer of a select list for formats.
...
Like the OAIS reference model itself, the PREFORMA conformance checker project aims to serve as a framework for standards-building activities through the creation of powerful reporting tools. By extension, MediaArea's MediaConch toolset enables the creation of a complex representation net in the OAIS Archive, providing information needed to adequately preserve audiovisual data objects through time.
...
MediaArea will utilize GitHub as a social and development center for MediaConch development and uses GitHub's issue tracker and wiki features alongside development.
